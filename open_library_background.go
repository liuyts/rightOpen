...
func CreateOpenLibraryBackground() {
	var openName string
	var openPath string
	...
	showPath := fmt.Sprintf(`LibraryFolder\background\shell\%s`, registryName)
	commandPath := fmt.Sprintf(`LibraryFolder\background\shell\%s\command`, registryName)
...
func DeleteOpenLibraryBackground() {
	fmt.Println("请输入打开程序的路径，后缀为.exe")
	scanner := bufio.NewScanner(os.Stdin)
	scanner.Scan()
	openPath := scanner.Text()
	...
	registryName := extractFileName(openPath)
	showPath := fmt.Sprintf(`HKEY_CLASSES_ROOT\LibraryFolder\background\shell\%s`, registryName)
